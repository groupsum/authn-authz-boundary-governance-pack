...
from oidc_authn_authz_boundary_governance_pack import (
...
class GovernancePackManifestTests(unittest.TestCase):
    def test_pack_metadata_contract_is_exposed(self) -> None:
        ...
        self.assertEqual("oidc-authn-authz-boundary-governance-pack", __ssot_package_name__)
        self.assertEqual("oidc-authn-authz-boundary-governance-pack", __pypi_package_name__)
        self.assertEqual("1.0.0", metadata["schema_version"])
        self.assertEqual("oidc-authn-authz-boundary-governance-pack", metadata["ssot_package_name"])
        self.assertEqual("pack:oidc-authn-authz-boundary", metadata["origin"]["id"])
        self.assertEqual("oidc_authn_authz_boundary_governance_pack", metadata["origin"]["import_name"])
        self.assertEqual("governance-pack", metadata["origin"]["kind"])
        self.assertEqual("extension-pack", metadata["trust"]["origin"])
        self.assertEqual("extension-pack:oidc-authn-authz-boundary-governance-pack", metadata["trust"]["reservation_owner"])
    ...
    def test_pack_manifest_contract_is_exposed(self) -> None:
        ...
        self.assertEqual("oidc-authn-authz-boundary-governance-pack", manifest["metadata"]["origin"]["package_name"])
        self.assertEqual(2, len(manifest["documents"]["adr"]))
        self.assertEqual(2, len(manifest["documents"]["spec"]))

    def test_packaged_adr_documents_are_downstream_portable(self) -> None:
        adr_manifest = load_document_manifest("adr")
        self.assertEqual("adr:authentication-and-authorization-are-separate-governance-surfaces", adr_manifest[0]["id"])
    ...
    def test_packaged_spec_documents_are_downstream_portable(self) -> None:
        ...
        self.assertEqual("spc:authn-authz-boundary-contract", spec_manifest[0]["id"])
